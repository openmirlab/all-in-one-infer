...
> **Note:** `all-in-one-infer` is an inference-only package — the training
> pipeline described below (the `allin1_infer.training` module, the `train`
> extra, and the `allin1_infer-train`/`allin1_infer-preprocess` CLI commands it
